...
# sensors = ["온도", "진동", "회전수", "압력"]
print()
# 검사일시+생산라인을 합쳐 키 생성 (두 표를 맞추기 위한 기준)
df["key"] = df["검사일시"] + df["생산라인"]
mentee_norm["key"] = mentee_norm["검사일시"] + mentee_norm["생산라인"]

# 문제 5까지 정리한 df를 표 전체 min-max로 정규화 (멘토 버전)
df_norm = df.copy()
for c in sensors:
    lo, hi = df_norm[c].min(), df_norm[c].max()
    df_norm[c] = ((df_norm[c] - lo) / (hi - lo)).round(4)

# key로 두 표 맞추기
merged = pd.merge(mentee_norm, df_norm, on="key", suffixes=("_멘티", "_멘토"))

# (1) 맞춰진 행 수
print(merged.shape[0])

# (2)(3) 센서별 차이 최댓값, 0.05 초과 개수
for c in sensors:
    diff = (merged[f"{c}_멘티"] - merged[f"{c}_멘토"]).abs()
    print(c, diff.max().round(4), (diff > 0.05).sum())

# (4) 온도 차이 상위 4개
merged["온도차이"] = (merged["온도_멘티"] - merged["온도_멘토"]).abs()
top4 = merged.sort_values("온도차이", ascending=False).head(4)
print(top4[["key", "온도_멘티", "온도_멘토"]])

# (5) 라인별 정규화 온도 평균 (멘티/멘토)
line_compare = (
    merged.groupby("생산라인_멘티")[["온도_멘티", "온도_멘토"]].mean().round(3)
)
print(line_compare)

# merge 결과 행수 확인 및 원인 분석
# --------------------------------
# 멘토(df) 원래 행수: 180 (A:60, B:60, C:60)
# 멘티정규화 원래 행수: 179 (A:60, B:62, C:57)
# merge 결과: 179행, 라인별로는 A:60, B:62, C:57
#
# 처음엔 "멘토가 180인데 멘티(179)에 맞춰 179가 됐다"로 보이지만,
# 실제 merge 결과 라인별 분포(A:60, B:62, C:57)를 뜯어보면
# 단순히 멘티 쪽에 맞춰 줄어든 게 아니라, merge 과정에서
# key 설계 자체의 결함으로 행이 늘고 준 것이 겹쳐서 나온 결과다.
#
# 1) C라인이 60 -> 57로 줄어든 것
#    문제5에서 나(멘토)는 압력 이상치를 "삭제"가 아니라 "라인 중앙값
#    으로 교체"했기 때문에 행 자체는 살아있다(60행 유지). 반면 멘티는
#    문제7~8에서 이 이상치 3개를 완전히 "삭제"했다. 그래서 멘토 쪽
#    C라인 3개 행은 멘티 쪽에 매칭될 key가 아예 없어 merge에서
#    자동으로 빠진다 (60 -> 57).
#
# 2) B라인이 60인데 merge 후 62로 늘어난 것 (기대와 다른 부분)
#    key를 "검사일시+생산라인"으로만 만들었는데, 이건 문제2에서 확인한
#    "같은 시각 같은 라인, 다른 설비"인 준중복 상황을 구분하지 못한다.
#    나(멘토)는 문제2에서 이 준중복을 정리해 B라인이 60행이지만,
#    멘티는 정규화_멘티.csv를 만들 때 이 준중복을 정리하지 않아
#    B라인에 같은 key가 2번씩 나오는 행이 2쌍(4행) 남아 있다.
#    merge는 key가 같으면 무조건 짝을 짓기 때문에, 멘토의 B라인 행
#    1개가 멘티 쪽 중복된 2개 행 모두와 매칭되면서 결과가 부풀려진다
#    (many-to-many merge) -> B라인이 60에서 62로 늘어난 것.
#
# 결론: 179행이라는 숫자와 A:60/B:62/C:57이라는 분포는 멘티 파일에
# "맞춰진" 것이 아니라, (a) C라인은 이상치 삭제 vs 교체 방식 차이로
# 실제로 매칭이 안 돼 빠진 것이고 (b) B라인은 멘티가 준중복을 정리
# 안 해서 key 중복으로 인해 오히려 부풀려진 것이다. 우연히 멘티의
# 원래 라인별 개수(60/62/57)와 똑같아 보이지만 원인은 전혀 다르다.
# key를 검사일시+생산라인+설비번호 세 열로 만들었다면 이런 왜곡 없이
# 정확히 177행(진짜 공통 행 수)으로 매칭됐을 것이다.

# [멘티에게 한 문장으로]
# "같은 A라인 기록인데 0.061이 아니라 0.536으로 찍힌 이유는, 결측치를
# 채울 때 A라인 평균이 아니라 표 전체 평균(84.4도, B/C라인까지 섞인 값)을
# 썼기 때문이다. 왜곡된 원본값을 그대로 정규화하니 A라인 정상 범위
# (0.1 근처)를 크게 벗어난 값이 나왔고, 이는 라인별 정규화 온도 평균
# 비교(A라인 0.132 vs 0.109)에서도 확인된다. 그리고 이번 비교 자체도
# 검사일시+생산라인만으로 키를 만들면 문제2의 준중복이 다시 살아나
# 매칭이 부정확해진다는 걸 발견했다 - 두 표를 비교할 땐 설비번호까지
# 포함한 완전한 키를 써야 한다."
